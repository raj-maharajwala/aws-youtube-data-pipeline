...
| Stage         | Main services                                                   | Purpose                                                                                                               | Output                                                                        |
| ------------- | --------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| Bronze        | AWS Lambda, Amazon S3, YouTube Data API, Kaggle dataset youtube | Ingest live YouTube trending data and category reference data, with optional historical backfill from Kaggle. youtube | Raw JSON and CSV data stored in S3. youtube                                   |
| Silver        | AWS Lambda, AWS Glue, PySpark, Amazon S3 youtube                | Clean, standardize, and transform raw files into analytics-friendly Parquet datasets. youtube                         | Cleansed Parquet tables for statistics and reference data. youtube            |
| Data Quality  | AWS Lambda, Amazon SNS youtube                                  | Validate row counts, null rates, schema expectations, value ranges, and freshness before promotion to Gold. youtube   | Pass/fail quality gate with alert notifications. youtube                      |
| Gold          | AWS Glue, PySpark, Amazon S3, AWS Glue Data Catalog youtube     | Build business-ready aggregations for trending, channel, and category analytics. youtube                              | trending_analytics, channel_analytics, and category_analytics tables. youtube |
| Query & BI    | Amazon Athena, Amazon QuickSight youtube                        | Query curated data and visualize insights through SQL-based analysis and dashboards. youtube                          | Analytics queries, reports, and dashboards. youtube                           |
| Orchestration | AWS Step Functions, Amazon EventBridge, Amazon SNS youtube      | Coordinate ingestion, transformation, quality checks, aggregation, retries, and notifications on a schedule. youtube  | End-to-end automated pipeline execution. youtube                              |
